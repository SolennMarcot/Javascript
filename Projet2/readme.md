...
- Tout à été réalisé : les tâches sont triées par ordre de priorité modifiable en un clic sur la case correspondante . L'appui sur le bouton rouge en forme de V sur la droite d'une tâche permet de la marquer comme "finie" . Le cas échéant , cette tâche est alors retirée de la zone des tâches en cours , et insérée dans la zone des tâches finies . Par ailleurs , un clic sur le bouton indiquant le nombre de tâches finies permet de dérouler la liste des tâches complétées , et , un nouveau clic la ferme .

Des tâches peuvent être ajoutées grâce aux zones de saisie en haut . On peut y inscrire le nom d'une tâche , ainsi que sa durée en minutes . Pour l'insérer dans les tâches en cours , il suffit d'appuyer sur le bouton d'ajout situé à droite des zones de saisie .

Pour finir , un filtre est disponible et permet de gérer l'affichage des tâches voulues , il suffit d'y inscrire du texte , et les tâches dont la description inclut ce texte sont affichées , les autres masquées .